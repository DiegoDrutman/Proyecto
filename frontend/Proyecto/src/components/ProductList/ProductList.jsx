...
const ProductList = ({ businessId }) => {
    ...
    const fetchProducts = async () => {
      if (businessId) {
        try {
          ...
          setMessage({ type: 'error', text: `Error al cargar los productos: ${error.message}` });
        }
      }
    };
    fetchProducts();
  ...
  const handleAddProduct = async () => {
    ...
    try {
      ...
      setMessage({ type: 'error', text: `Error al agregar el producto: ${error.message}` });
    } finally {
...
ProductList.propTypes = {
  businessId: PropTypes.number.isRequired,  // Validar que businessId sea un número
};
